...
export default function InvoiceTable({ 
  ...
  const handleDeletePropertyNote = (propertyName: string) => {
    if (confirm('Are you sure you want to delete this property note? This will remove it from all invoices for this property.')) {
      onDeletePropertyNote(propertyName);
      setViewingPropertyNote(null);
    }
  };
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {invoices.map((invoice) => {
              ...
              const isHovered = hoveredPropertyNote === invoice.propertyName;
              ...
              return (
                <React.Fragment key={invoice.invoice_id}>
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className={`px-4 py-4 relative ${hasPropertyNote ? 'bg-yellow-100' : ''}`}>
                      <div className="flex justify-center">
                        {invoice.propertyName ? (
                          <div className="relative">
                            <button
                              onClick={() => handlePropertyNoteClick(invoice.propertyName!)}
                              onMouseEnter={() => setHoveredPropertyNote(invoice.propertyName!)}
                              onMouseLeave={() => setHoveredPropertyNote(null)}
                            ...
                            {/* Hover Tooltip */}
                            {isHovered && hasPropertyNote && (
                              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 z-50 w-64 bg-gray-900 text-white text-xs rounded-lg p-3 shadow-lg pointer-events-none">
                                <div className="font-semibold mb-1">Property Note:</div>
                                <div className="max-h-32 overflow-y-auto whitespace-pre-wrap">
                                  {propertyNote.note_text}
                                </div>
                                <div className="text-gray-400 mt-2 text-[10px]">
                                  By {propertyNote.created_by} • Click to edit/delete
                                </div>
                                {/* Arrow */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4">
            ...
            <div className="p-6 border-t border-gray-200 flex gap-3">
              ...
              <button
                onClick={() => handleDeletePropertyNote(viewingPropertyNote)}
              ...
              <button
                ...
                className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded text-sm font-medium transition-colors ml-auto"
              >
                Close
              </button>
            </div>
